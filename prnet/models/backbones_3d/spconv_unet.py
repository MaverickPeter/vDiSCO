...
class UNetV2(nn.Module):
    ...
    def __init__(self, model_cfg, input_channels, grid_size, voxel_size, point_cloud_range, **kwargs):
        ...
        if model_cfg.use_rgb and model_cfg.use_xyz:
            self.conv5 = spconv.SparseSequential(
                block(16, model_cfg.feature_dim//2, 3, norm_fn=norm_fn, padding=1, indice_key='subm1')
            )
            self.num_point_features = model_cfg.feature_dim//2
        else:
            self.conv5 = spconv.SparseSequential(
                block(16, model_cfg.feature_dim, 3, norm_fn=norm_fn, padding=1, indice_key='subm1')
            )
    ...
    def forward(self, batch_dict):
        ...
        batch_dict['encoded_voxel_features'] = x_up1
