...
def phase_corr(a, b):
    # a: template spectrum; b: source spectrum
    corr = torch.fft.ifft2(a*(b.conj()), dim=(-3, -2), norm="ortho") 
    corr = torch.sqrt(corr.real**2 + corr.imag**2)
    corr = torch.sum(corr, dim=-1)
    corr = torch.fft.fftshift(corr)
    # import matplotlib.pyplot as plt
    # plt.imshow(corr.squeeze(0).detach().cpu().numpy())
    # plt.show()
    # plt.savefig("/mnt/workspace/corr.png")
    corr = torch.sum(corr, dim=1)
    corr = corr.squeeze()
